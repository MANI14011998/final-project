...
class ChatViewController: JSQMessagesViewController {
    ...
    func addMicButton()
    {
        ...
        micButton.setImage(#imageLiteral(resourceName: "Image"), for: .normal)
    ...
    {
        self.addMessage(withId: "BotId", name: "Bot", text: "Hi I am Doctor Jokes")
        self.finishReceivingMessage()
        self.addMessage(withId: "BotId", name: "Bot", text: "I will made you laugh")
        self.finishReceivingMessage()
        self.addMessage(withId: "BotId", name: "Bot", text: "At any moment you feel that I am troubling you, you can ask me .")
        self.finishReceivingMessage()
    }
    ...
    func performQuery(senderId:String,name:String,text:String)
    {
        ...
        request?.setMappedCompletionBlockSuccess({ (request, response) in
            ...
            else if response.result.action == "bot.quit"
            {
                if let parameters = response.result.parameters as? [String:AIResponseParameter]
                {
                    if (parameters["quit"]?.stringValue) != nil
                    {
                        let deadlineTime = DispatchTime.now() + .seconds(2)
                        DispatchQueue.main.asyncAfter(deadline: deadlineTime, execute: {
                            self.minimiseBot()
